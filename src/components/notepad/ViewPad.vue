            ...
            <template :key="item.id" v-for="item in getActiveNote.content">
                ...
                <template v-if="item.type === types.text_link">
                    ...
                    <template v-else>
                        ...
                        <a target="blank" class=" underline underline-offset-1 text-accent" :href="item.url" v-else>{{item.text}}</a>
                    </template>
                </template>
                ...
                <template v-if="item.type === types.image">
                    <template v-if="noteseditmode">
                        ...
                        <input :ref="`${item.id}_img_input`" class="hidden" type="file" name="" id="">
                    </template>
                    <template v-else>
                        <div class="relative bg-gray-200 h-max rounded-md">
                            <img
                                loading='lazy'
                                @error="$refs[`${item.id}_img_error`][0].style.display='inline';"
                                ...
                                :src="item.src" alt="">
                            <!-- load -->
                            <div :ref="`${item.id}_img_load`" class="transition delay-200 absolute top-0 left-0 right-0 bottom-0">
                                <div class="w-full h-full flex items-center justify-center">
                                    <div class="bg-white shadow-lg flex gap-4 p-3 rounded-md items-center justify-center">
                    ...
                    <template v-if="noteseditmode">
                        <div :class="[(activeContentItem===item.id) ? 'border-accent' : ' border-gray-200']" class="border-4 p-2 rounded-md">
                            ...
                            <div :key="subitem.id" v-for="subitem in item.items" class="flex gap-3 group items-center hover:bg-accent hover:bg-opacity-10 pl-2">
                                ...
                                <!-- type -->
                                <group-button label="" class="group-hover:w-max w-0 border-0 group-hover:border-2 overflow-x-clip">
                                    <group-button-item @click="subitem.listType='checklist'" class="rounded-l-md" :class="[(subitem.listType=='checklist')?'text-white bg-accent hover:bg-accent':'bg-white text-text']">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M15 19.411L12.3 16.711L13.714 15.295L15 16.583L20.008 11.583L21.419 13L15 19.41V19.411ZM11 17H2V15H11V17ZM15 13H2V11H15V13ZM15 9H2V7H15V9Z" fill="currentColor"/>
                                        </svg>
                                    </group-button-item>
                                    <group-button-item @click="subitem.listType='normal'" class="rounded-r-md" :class="[(subitem.listType=='normal')?'text-white bg-accent hover:bg-accent':'bg-white text-text']">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M8 5H21V7H8V5ZM3 4.5H6V7.5H3V4.5ZM3 10.5H6V13.5H3V10.5ZM3 16.5H6V19.5H3V16.5ZM8 11H21V13H8V11ZM8 17H21V19H8V17Z" fill="currentColor"/>
                                        </svg>
                                    </group-button-item>
                                </group-button>
                                <!-- control -->
                                <group-button class="border-0 group-hover:border-2 group-hover:w-max w-0 overflow-x-clip" label="">
                                    <group-button-item class="bg-white" @click="swapListItem(item.items, subitem, -1), $store.commit('SET_CURRENT_CONTENT_ITEM', item.id)">
                                    ...
                                    <group-button-item class="bg-white" @click="subitem.indent = (subitem.indent>0) ? subitem.indent-1 : 0, $store.commit('SET_CURRENT_CONTENT_ITEM', item.id)">
                                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M8.25 12.75L4.5 9M4.5 9L8.25 5.25M4.5 9L13.5 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                    </group-button-item>
                                    <group-button-item class="bg-white" :stroke="false" @click="subitem.indent = (subitem.indent<3) ? subitem.indent+1 : 3, $store.commit('SET_CURRENT_CONTENT_ITEM', item.id)">
                                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M9.75 5.25L13.5 9M13.5 9L9.75 12.75M13.5 9L4.5 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                    </group-button-item>
                                </group-button>
                            </div>
                            ...
                            <div class="flex bg-gray-200 gap-4 rounded-md">
                                <input @click="$store.commit('SET_CURRENT_CONTENT_ITEM', item.id)" @keypress.enter="addListitem(item.items, $event.target)" :ref="`${item.id}_new_listitem_input`" placeholder="enter text..." type="text" name="" class="px-3 py-2 bg-transparent outline-none flex-grow">
                                <button @click="addListitem(item.items, $refs[`${item.id}_new_listitem_input`][0]), $store.commit('SET_CURRENT_CONTENT_ITEM', item.id)" class="bg-accent bg-opacity-10 rounded-r-md text-accent hover:bg-opacity-100 hover:text-white  transition-all duration-200 py-2 px-4">add</button>
                            </div>
                        </div>
                    </template>
                    <template v-else>
                        <div>
                            <!-- item component -->
                            <div :key="subitem.id" v-for="subitem in item.items" class="flex gap-3 group items-center">
                                <!-- item -->
                                <div class="flex flex-grow items-center">
                                    ...
                                    <div class="flex gap-3 h-8 items-center">
                                        <!-- check state -->
                                        <div @click="subitem.checked = !subitem.checked" v-if="subitem.listType=='checklist'" :class="[(subitem.checked) ? 'bg-accent': 'bg-gray-300']" class="cursor-pointer hover:p-[0.35rem] transition-all duration-200 p-1 h-max rounded">
                                            <svg :class="[(subitem.checked) ? 'text-white' : 'text-transparent']" class="" width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M5 13L9 17L19 7" stroke="currentColor" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/>
                                            </svg>
                                        </div>
                                        <!-- bullet -->
                                        <div v-if="subitem.listType=='normal'" class="w-2 h-2 rounded-full bg-text"/>
