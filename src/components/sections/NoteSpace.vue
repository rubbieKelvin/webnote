<template>
  <div
    :class="[
      $store.state.mobileWindow === 1 ? 'flex' : 'hidden',
      noteseditmode ? 'pb-16' : 'p-1',
    ]"
    ref="noteSpaceView"
    @keydown.esc="deleteModalOpen = false"
    @keypress.delete="deleteModalOpen = true"
    :tabindex="activeNote ? '0' : ''"
    class="
      md:pt-4
      pt-3
      border-white
      ...
      md:pb-1
      min-w-full
      md:max-w-[65%] md:min-w-[calc(65%-1px)]
      gap-3
      md:flex
      flex-col
      items-center
    "
  >
    <div class="md:hidden w-full flex gap-2 px-2 items-center font-dm text-xl">
      <button
        class="text-text px-3 py-1"
        @click="$store.state.mobileWindow = 0"
      >
        <svg
          width="24"
          height="24"
        ...
        >
          <path
            d="M15.535 3.51501L7.05 12L15.535 20.485L16.95 19.071L9.878 12L16.95 4.92901L15.535 3.51501Z"
            fill="currentColor"
          />
        </svg>
      </button>
    ...
    <div
      class="select-none px-2 w-full flex md:flex-row flex-col gap-6"
      :class="[
        activeNote === null
    ...
    >
      <group-button
        label=""
        class="md:w-auto w-full md:overflow-x-visible overflow-x-auto"
      >
        <group-button-item
          class="group"
        ...
        >
          ...
          >
            <path
              d="M6 5.25V11.25C6 12.0784 6.67157 12.75 7.5 12.75H12M6 5.25V3.75C6 2.92157 6.67157 2.25 7.5 2.25H10.9393C11.1383 2.25 11.329 2.32902 11.4697 2.46967L14.7803 5.78033C14.921 5.92098 15 6.11175 15 6.31066V11.25C15 12.0784 14.3284 12.75 13.5 12.75H12M6 5.25H5C3.89543 5.25 3 6.14543 3 7.25V14.25C3 15.0784 3.67157 15.75 4.5 15.75H10C11.1046 15.75 12 14.8546 12 13.75V12.75"
              stroke="currentColor"
              stroke-width="1.5"
        ...
        <group-button-item
          class="group"
          :title="noteseditmode ? 'save [Ctrl+S]' : 'enter edits mode [Ctrl+E]'"
        ...
        <group-button-item
          ...
          :class="{
            'md:text-white text-accent bg-white md:bg-accent hover:md:bg-accent':
              getActiveNote?.favorite,
          }"
          @click="
            $store.commit('UPDATE_ACTIVE_NOTE', {
              favorite: !getActiveNote.favorite,
            })
          "
        >
          <svg
        ...
        >
          ...
          <p class="text-xs md:text-sm items-center flex gap-2">
            <span>Export</span>
            <span class="shortcut-snack">Ctrl+Shift+E</span>
          </p>
        </group-button-item>
      ...
      >
        ...
        >
          ...
          >
            <path
              d="M6.75 9H11.25M6.75 12H11.25M12.75 15.75H5.25C4.42157 15.75 3.75 15.0784 3.75 14.25V3.75C3.75 2.92157 4.42157 2.25 5.25 2.25H9.43934C9.63825 2.25 9.82902 2.32902 9.96967 2.46967L14.0303 6.53033C14.171 6.67098 14.25 6.86175 14.25 7.06066V14.25C14.25 15.0784 13.5784 15.75 12.75 15.75Z"
              stroke="currentColor"
              stroke-width="1.5"
              stroke-linecap="round"
              stroke-linejoin="round"
    ...
    <div class="md:mx-2 w-full bg-stroke" style="height: 1px" />
    ...
    <!-- overlay -->
    <div
      @focus="(deleteModalOpen = false), $refs.noteSpaceView.focus()"
      tabindex="0"
      v-if="activeNote && deleteModalOpen"
      class="
        select-none
        text-text
        absolute
    ...
    >
      <div
        @keydown.esc="(deleteModalOpen = false), $refs.noteSpaceView.focus()"
        tabindex="0"
        class="bg-white p-4 rounded-md shadow-md flex flex-col gap-2"
      >
        ...
        <div class="flex flex-row gap-3">
          <button
            @click="(deleteModalOpen = false), $refs.noteSpaceView.focus()"
            class="
              bg-gray-200
              px-4
              rounded-md
          ...
          <button
            @click="
              $store.commit('UPDATE_ACTIVE_NOTE', { trash: true }),
                (deleteModalOpen = false),
                ($store.state.notebook.activeNote = null)
            "
            class="
              bg-gray-200
              px-4
          ...
          <button
            @click="
              $store.commit('DELETE_CURRENT_NOTE'),
                (deleteModalOpen = false),
                $refs.noteSpaceView.focus()
            "
            class="
              px-4
              rounded-md
...
<style scoped>
.shortcut-snack {
  @apply md:flex hidden text-xs bg-gray-200 text-gray-700 group-hover:p-1 rounded-md group-hover:w-auto transition-all duration-200 overflow-clip w-0;
}
</style>
