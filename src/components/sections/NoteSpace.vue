<template>
    <div
    :class="[($store.state.mobileWindow===1)?'flex':'hidden', (noteseditmode)?'pb-16':'p-1']"
    ref="noteSpaceView"
    @keydown.esc="deleteModalOpen=false"
    @keypress.delete="deleteModalOpen=true"
    :tabindex="activeNote?'0':''"
    class="md:pt-4 pt-3 border-white relative focus:border-blue-300 md:border-2 outline-none flex-grow md:pb-1 min-w-full md:max-w-[50%] md:min-w-[calc(50%-1px)] gap-3 md:flex flex-col">
        <div class="md:hidden flex gap-2 px-2 items-center font-dm text-xl">
            <button class="text-text px-3 py-1" @click="$store.state.mobileWindow=0">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M15.535 3.51501L7.05 12L15.535 20.485L16.95 19.071L9.878 12L16.95 4.92901L15.535 3.51501Z" fill="currentColor"/>
                </svg>
            </button>
            <h1>Edit Note</h1>
        </div>
        
        <div class=" select-none px-2 flex md:flex-row flex-col gap-6" :class="[activeNote===null ? ' text-gray-400 pointer-events-none' : 'text-text']">
            <group-button label="" class="md:w-auto w-full md:overflow-x-visible overflow-x-auto">
                ...
                <group-button-item class="group" :title="(noteseditmode) ? 'save [Ctrl+S]': 'enter edits mode [Ctrl+E]'" @click="toggleEditMode()">
                ...
                <group-button-item class="group" title="mark as favourite" :class="{'md:text-white text-accent bg-white md:bg-accent hover:md:bg-accent': getActiveNote?.favorite}" @click="$store.commit('UPDATE_ACTIVE_NOTE', {favorite: !getActiveNote.favorite})">
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <group-button-item class="group" title="Add Comment" @click="setCommentBoxVisibility(!showCommentModal)">
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <group-button v-if="noteseditmode" label="" class="md:static fixed bottom-0 left-0 right-0 w-full md:w-auto h-16 bg-white">
        ...
        <div class="md:mx-2 bg-stroke" style="height: 1px"/>
