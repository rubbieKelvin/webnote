    ...
    <div :class="[($store.state.mobileWindow===0)?'flex':'hidden']" class="md:border-r border-stroke select-none md:flex gap-2 flex-col text-text flex-grow md:min-w-[20%] md:max-w-[20%] min-w-full py-4">
        ...
        <div class="text-text h-0 scrollbar overflow-y-auto md:overflow-y-hidden hover:md:overflow-y-auto flex flex-col flex-grow">
            ...
            <template v-for="note in notes" :key="note.id || note.local_id">
                ...
                class="px-5 md:px-2 pt-2 cursor-pointer group flex flex-col gap-1 md:gap-2 outline-none focus:md:border-accent border-transparent border-l-2 rounded-md md:rounded-none">
                    ...
                    <div class="flex flex-row gap-3 items-center">
                        <p class="text-sm flex-grow text-gray-400">{{prettyDate(note.last_edited)}}</p>
            ...
            <div v-if="notes.length===0 && !new_note_modal_visible" class="flex-grow flex flex-col items-center justify-center gap-2">
                <h1>No Note</h1>
                <p class="text-sm text-gray-500 hidden md:inline">You dont have any notes here</p>
